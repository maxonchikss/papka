...
def cmd_ls(args):
    ...
    if node["type"] != "dir": print(path); return
    for name in node["children"]: print(name)
#change directory
def cmd_cd(args):
    global CWD
    if not args: CWD = []; return
    _, res = resolve(args[0])
    CWD = res
#print working directory
def cmd_pwd(_): print("/" + "/".join(CWD))
#concatenate
def cmd_cat(args):
    if not args: return
...
def main():
    ...
    if len(sys.argv)>1: VFS_PATH=sys.argv[1]
    if len(sys.argv)>2: START_SCRIPT=sys.argv[2]
    load_vfs(VFS_PATH)
